...
class SimaiHold(SimaiSimpleNote):
    def __init__(self, cursor: tuple[int, int, str], moment: float, idx: int, duration: float):
        ...
        self.tail_on_slide_head = False     # 表示这个hold结束的时候正好有一条slide启动，拍-按-划

    def set_tail_on_slide_head(self, tail_on_slide_head: bool):
        self.tail_on_slide_head = tail_on_slide_head
...
class SimaiWifi(SimaiNote):
    def __init__(self, cursor: tuple[int, int, str], moment: float, shape: str, wait: float, duration: float):
        ...
        self.area_judge_actions: "list[list[tuple[Action, float] | None]]" = [[None] * self.total_area_num for _ in
                                                                              range(3)]
